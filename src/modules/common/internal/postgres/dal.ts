...
export class DALImpl implements DAL {
  ...
  constructor(options: DALOptions, private readonly logger: Logger) {
    if (instance === null) {
      const pg = pgPromise({ noWarnings: true });
      const connectionOptions = {
        connectionString: options.databaseUrl,
        max: options.pgMaxConnectionPool ?? this.maxSqlExecTime,
      };
...
export default function DALFactory(options: DALOptions): DAL {
 return new DALImpl(options, new Logger());
};